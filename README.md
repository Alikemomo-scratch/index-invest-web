...
- 查看 PE、PB、股息率、盈利收益率溢价和股息率溢价的当前值、历史分位、来源和更新时间；A 股股息率同时展示历史覆盖和官方 D/P1、D/P2 核对值。
- 估值折线支持日、月、季度、年四种显示粒度，鼠标或触控滑动可查看具体坐标。
- 按月、季度或半年生成历史买入点，研究持有 1、3、5、10 年的年化或累计收益。
- 收益研究器展示平均值、中位数、标准差、盈利样本占比、最差样本和有效样本数。
...
| 用途 | 数据源 | 说明 |
| --- | --- | --- |
| A 股 PE、股息率当前口径和指数行情 | 中证指数官网 | 官方数据优先；股息率保留 D/P1（总股本）与 D/P2（计算用股本） |
| A 股股息率历史 | FundDB / 韭圈儿 | 公共聚合历史；用于同源历史分位，精确 D/P1/D/P2 映射未公开，页面与官方当前值分列 |
| A 股 PB 与 PE 交叉核验 | 乐咕乐股 | 公共聚合数据，逐项标记来源；不可用时不伪造替代值 |
| 标普 500 估值 | Multpl | 公共历史表，估算观察保留 `estimated` 标记 |
| 中美 10 年期国债收益率 | 东方财富数据中心 | 公共聚合历史；A 股匹配中国 10 年期，美股匹配美国 10 年期，来源与指数端分别披露 |
...
- 股息率口径：公共聚合历史自身保持连续，不与中证官方当前值拼接；官方 D/P1、D/P2 只作同期核对。页面会报告可用起止日期、有效月份、缺失月份和回看期是否被截短。
- 风险溢价：盈利收益率溢价为 `100 / PE TTM - 10 年期国债收益率`，股息率溢价为 `指数股息率 - 10 年期国债收益率`。各组成序列先独立取月末有效值，再只按相同自然月连接；零值和负值均保留。数值及分位越高表示相对无风险利率的补偿越高。
- 买入时点：所选月、季度或半年的最后一个有效交易日。
- 持有结束点：目标周年日当天或之前最近的有效交易日。
- 年化收益：`(期末 / 期初) ** (365.2425 / 实际天数) - 1`。
- 平均收益：当前所有完整持有样本收益率的算术平均值。
- 收益标准差：当前所有完整持有样本收益率的总体标准差，用于描述同一收益口径下的历史样本离散程度。
...
- 免费源无法保证所有指数都有完整的历史 PE、PB 和股息率；例如东证红利低波当前仅展示官方股息率，长期分位保持不可用并显示覆盖缺口。
