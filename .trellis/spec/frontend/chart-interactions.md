...
- Holding-return summary cards display both arithmetic average and median for the same complete samples. Their labels switch between annualized and cumulative wording with the active measure.
...
- Good: return hover displays the exact sample's buy date, end date, and annualized or cumulative value while both crosshair guides track the point.
...
- Frontend contract and browser smoke tests assert the average card renders the API `summary.average` value and changes its label with the active measure.
